                ...
                <v-expansion-panels>
                    <v-expansion-panel class="bg-color-211F31 mt-2" v-for="(item, index) in bonusList" :key="index">
                        <v-expansion-panel-title :class="[item.type == 'Failure' ? 'failure-title' : 'completion-title']">
                            <template v-slot:default="{ expanded }">
                                <v-row no-gutters class="align-center">
                                    <v-col cols="2" class="text-700-18 text-center"
                                        :class="[item.type == 'Failure' ? 'black' : '']">{{ item.restCash }}</v-col>
                                    <v-col cols="2" class="text-400-16 text-center"
                                        :class="[item.type == 'Failure' ? 'color-FF6200' : '']">{{ item.type }}</v-col>
                                    <v-col cols="5" class="text-center">
                                        <div class="text-400-14" v-if="item.type != 'Underway'"
                                        ...
                                        <div :class="[item.type != 'Underway' ? 'mt-2' : '']">
                                            <v-progress-linear v-model="item.rate" height="21"
                                                ...
                                                <div class="text-500-12" :class="[item.type == 'Failure' ? 'gray' : '']">{{
                                                    item.currentCash }} / {{ item.totalCash }}</div>
                                            </v-progress-linear>
                                        </div>
                                    </v-col>
                                    <v-col cols="2" class="text-center" v-if="item.type != 'Underway'">
                                        <div class="text-400-12" :class="[item.type == 'Failure' ? 'black' : '']">Expire on
                                        </div>
                                        <div class="text-600-14 mt-2" :class="[item.type == 'Failure' ? 'black' : '']">{{
                                            item.expireDate }}</div>
                                    </v-col>
                        ...
                        <v-expansion-panel-text>
                            ...
                            <v-row class="ma-1">
                                <v-col cols="6" class="text-left text-500-14">
                                    ID: Qsd58844221122
                                </v-col>
